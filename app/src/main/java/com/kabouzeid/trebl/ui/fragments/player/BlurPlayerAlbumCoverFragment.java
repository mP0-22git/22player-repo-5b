...
import com.kabouzeid.trebl.adapter.BlurAlbumCoverPagerAdapter;
import com.kabouzeid.trebl.helper.MusicPlayerRemote;
import com.kabouzeid.trebl.helper.MusicProgressViewUpdateHelper;
import com.kabouzeid.trebl.misc.SimpleAnimatorListener;
import com.kabouzeid.trebl.model.lyrics.AbsSynchronizedLyrics;
 ...
 */
public class BlurPlayerAlbumCoverFragment extends AbsMusicServiceFragment implements ViewPager.OnPageChangeListener, MusicProgressViewUpdateHelper.Callback {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_blur_player_album_cover, container, false);
    ...
    private void updatePlayingQueue() {
        ...
        viewPager.setAdapter(new BlurAlbumCoverPagerAdapter(getChildFragmentManager(), MusicPlayerRemote.getPlayingQueue()));
        viewPager.setCurrentItem(MusicPlayerRemote.getPosition());
    ...
    public void onPageSelected(int position) {
        ...
        ((BlurAlbumCoverPagerAdapter) viewPager.getAdapter()).receiveColor(colorReceiver, position);
        if (position != MusicPlayerRemote.getPosition()) {
            MusicPlayerRemote.playSongAt(position);
        }
    }

    private BlurAlbumCoverPagerAdapter.RoundedAlbumCoverFragment.ColorReceiver colorReceiver = new BlurAlbumCoverPagerAdapter.RoundedAlbumCoverFragment.ColorReceiver() {
        @Override
    ...
    private void hideLyricsLayout() {
        lyricsLayout.animate().alpha(0f).setDuration(BlurPlayerAlbumCoverFragment.VISIBILITY_ANIM_DURATION).withEndAction(() -> {
    ...
    public void setLyrics(Lyrics l) {
        ...
        lyricsLayout.animate().alpha(1f).setDuration(BlurPlayerAlbumCoverFragment.VISIBILITY_ANIM_DURATION);
    }
    ...
    public void onUpdateProgressViews(int progress, int total) {
        ...
        if (!oldLine.equals(line) || oldLine.isEmpty()) {
            ...
            lyricsLine1.animate().alpha(0f).translationY(-h).setDuration(BlurPlayerAlbumCoverFragment.VISIBILITY_ANIM_DURATION);

            lyricsLine2.setAlpha(0f);
            lyricsLine2.setTranslationY(h);
            lyricsLine2.animate().alpha(1f).translationY(0f).setDuration(BlurPlayerAlbumCoverFragment.VISIBILITY_ANIM_DURATION);
        }
    }
